...
df_gap_no <- df_gap_no %>%
  mutate(sex_residual_n = lm(sex ~ age+estrato+maxEducLevel+
                             hoursWorkUsual+microEmpresa , data = df_gap_no)$residuals)

# hacer una regresión con la variable y=log_y_salary_h en X2-X6 y tomar los residuos

df_gap_no <- df_gap_no %>%
  mutate(salary_residual_n = lm(log_y_salary_h ~ age+maxEducLevel+estrato+
                                hoursWorkUsual+microEmpresa , data = .)$residuals)

# hacer una regresión entre salary_residual y sex_residual 
reg3.2 <- lm(salary_residual_n ~ sex_residual_n, data = df_gap_no )
stargazer(reg2.2, reg3.2, type = "text", digits = 7)

# estimaciones del modelo FWL 

coef_FWL <- reg3.2$coefficients
coef_FWL

# comprobamos residuos SS
sum(resid(reg2.2)^2) 
sum(resid(reg3.2)^2) # residuos iguales 

# comprobamos el error estándar 
sqrt(diag(vcov(reg2.2)))[2] 
sqrt(diag(vcov(reg3.2)))[2]
...
set.seed(123)
B <- 2000 # Número de repeticiones 
e_reg3.2 <- rep(0,B) # vector donde gurdaremos las estimaciones 
...
for(i in 1:B){
  ...
  m2<-lm(salary_residual_n ~ sex_residual_n,data= db_sample)
  coefs<-m2$coefficients[2] 
  e_reg3.2[i]<-coefs 
}
# check b=2000
length(e_reg3.2)
...
boot_i<-boot(sub_set_sex, funcion_pa_sex, R=2000) # base datos con na imputados
...
i_c_w$bootstrap_i<-boot_i[["t"]]
i_c_w$l_i_i<-boot_i[["t"]]-qnorm(0.05/2)*se_i
i_c_w$l_s_i<-boot_i[["t"]]+qnorm(0.05/2)*se_i
...
#Hacemos el boot de forma automática con nuestra función
boot_x<-boot(sub_set_m, funcion_pa_sex, R=2000) # base datos con na imputados
